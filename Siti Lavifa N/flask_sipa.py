...
def handle_message(client, userdata, msg):
    ...
    if topic == MQTT_CONFIG["topics"]["motion"]:
        current_data['motion'] = float(message)
    
    if 'motion' in current_data:
        record = {
            'timestamp': now,
            'motion': current_data['motion']
        }
...
@app.route('/datasipaa', methods=['POST'])
def insert_dummy_data():
    ...
    return jsonify({"message": "Data Masuk!"}), 200

@app.route('/datasipaa', methods=['GET'])
def retrieve_data():
